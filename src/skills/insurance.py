...
class InsuranceSkill:
    """
    Skill set for insurance plan recommendation and comparison.
    """

    name = "insurance_plan"
    description = "提供团险方案推荐、筛选和多方案对比功能"
    keywords = ["方案", "推荐", "对比", "计划", "保障", "价格", "预算"]
    ...
    def recommend_insurance_plans(customer_info: str, budget: int) -> str:
        ...
        返回多个方案选项。
        """
        print(f"\n[InsuranceSkill] Recommending plans for budget: {budget}")
        # Mock plans
        plans = [
            {
                "id": "PLAN_A",
    ...
    def compare_insurance_plans(plan_ids: List[str]) -> str:
        ...
        返回对比表格数据。
        """
        print(f"\n[InsuranceSkill] Comparing plans: {plan_ids}")
    ...
    @staticmethod
    def get_tools():
        return [InsuranceSkill.recommend_insurance_plans, InsuranceSkill.compare_insurance_plans]
